...
class RagDaemon:
    ...
    async def start(self) -> None:
        ...
        logger.info("  host=%s  port=%d  idle_timeout=%d", self._host, self._port, self._idle_timeout)
        ...
        if active:
            logger.info("Active RAG: '%s' (model=%s, folders=%s)", active.name, active.embedding_model, active.source_folders)
            ...
            if incomplete:
                ...
                self._loop.call_soon(
                    lambda: asyncio.ensure_future(self._recovery_reindex())
                )
    ...
    async def _recovery_reindex(self) -> None:
        ...
        try:
            ...
            logger.info(
                ...
                state.status, state.processed_files, state.total_chunks,
                state.duration_seconds,
            )
        except Exception as exc:
            logger.error("Recovery reindex failed: %s", exc)
    ...
    async def _handle_client(
        ...
        logger.info("Client connected: %s (active_connections=%d)", peer, self._active_connections + 1)
        ...
        try:
            while True:
                ...
                try:
                    ...
                    elapsed = time.monotonic() - t0
                    logger.warning("[%s] ← %s ERROR %d: %s (%.3fs)", peer, method, exc.code, exc, elapsed)
                    response = make_error(request_id, exc.code, str(exc))
                except Exception as exc:
                    elapsed = time.monotonic() - t0
                    logger.exception("[%s] ← %s UNHANDLED ERROR (%.3fs): %s", peer, method, elapsed, exc)
            ...
            try:
                ...
                pass
            logger.info("Client disconnected: %s (active_connections=%d)", peer, self._active_connections)
    ...
    async def _rag_create(self, params: dict, rid: str | None, w) -> Any:
        ...
        logger.info("Creating RAG '%s' (folders=%s, model=%s)", name, params.get("folders"), params.get("embedding_model"))
    ...
    async def _rag_switch(self, params: dict, rid: str | None, w) -> Any:
        ...
        try:
            ...
            raise _AppError(ERR_RAG_NOT_FOUND, f"RAG '{name}' not found")
    ...
    async def _index_run(self, params: dict, rid: str | None, w: asyncio.StreamWriter) -> Any:
            ...
            **and** update the shared ``_indexing_state`` dict so that
            ...
            try:
                ...
                asyncio.run_coroutine_threadsafe(
                    write_frame_async(w, notification), loop,
                )
            except Exception:
                pass  # Don't let progress errors kill the indexing

        # Mark indexing as started
        logger.info("Indexing started: rag=%s full=%s workers=%s", rag_name or "(active)", full, workers)
        self._indexing_state = {
            "is_indexing": True,
            "status": "starting",
            "processed_files": 0,
        ...
        try:
            ...
            raise _AppError(ERR_INDEX_ERROR, str(exc))
        finally:
        ...
        logger.info(
            ...
            state.status, state.processed_files, state.total_files,
            len(state.errors), state.duration_seconds,
        )
        return {
            "status": state.status,
            "processed_files": state.processed_files,
    ...
    async def _index_status(self, params: dict, rid: str | None, w) -> Any:
        assert self._api is not None
        rag_name = params.get("rag_name")
        status = self._api.get_index_status(rag_name=rag_name)
        result = {
    ...
    async def _share_import(self, params: dict, rid: str | None, w) -> Any:
        ...
        try:
            ...
            raise _AppError(ERR_RAG_ALREADY_EXISTS, str(exc))
